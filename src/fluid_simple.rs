    ...
    pub fn step(&mut self) {
        ...
        for y in 1..self.height-1 {
            for x in 1..self.width-1 {
                let idx = y * self.width + x;
                new_density[idx] = self.density[idx] + self.diffusion * (
        ...
        for y in 1..self.height-1 {
            for x in 1..self.width-1 {
                let idx = y * self.width + x;
                new_vel_x[idx] = self.velocity_x[idx] + self.viscosity * (
                ...
                new_vel_y[idx] = self.velocity_y[idx] + self.viscosity * (
        ...
        for y in 1..self.height-1 {
            for x in 1..self.width-1 {
                ...
                // Backtrace position
                let src_x = (x as f32 - self.dt * new_vel_x[idx]).max(1.0).min((self.width - 2) as f32);
                let src_y = (y as f32 - self.dt * new_vel_y[idx]).max(1.0).min((self.height - 2) as f32);
