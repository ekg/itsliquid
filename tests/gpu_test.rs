...
use itsliquid::{gpu_minimal::MinimalGPUFluid, FluidSimulation};
...
async fn test_gpu_different_resolutions() {
    ...
    for (width, height) in resolutions.iter() {
        ...
        assert!(fluid.is_ok(), "Should create {}x{} GPU simulation", width, height);
...
async fn test_gpu_cpu_consistency() {
    use itsliquid::{fluid_interactive::InteractiveFluid, FluidSimulation};
